...
function renderBasketMarkup(data) {
  ...
  // console.log(countEventer(userBasket))
    ...
    data.forEach((item, index, array) => {
      if (array.indexOf(item) === index) return item
      let countEvent = array.filter(({id}) => id === item).length;
      let {name, images, _embedded: {venues}} = item;
      let {city: {name: nameCity}, country: {name: nameCountry}} = venues[0];
       markup += `<li class="event__item"><div class="event__container"><img src=${smallestPhoto(images)} class="event__img"><div><span>${countEvent}</span>>8 0 5.296 3.003 9 3.003 9S6 5.19 6 3.128C6 1.403 4.654 0 3 0Zm.905 4.044c-.25.26-.577.39-.905.39a1.25 1.25 0 0 1-.905-.39c-.5-.52-.5-1.367 0-1.887a1.246 1.246 0 0 1 1.81 0c.5.52.5 1.367 0 1.887Z"/></svg>
